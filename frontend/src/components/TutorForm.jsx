...
export default function TutorRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.agree) { setMessage("⚠️ Please agree to Terms."); return; }
    if (!formData.image) { setImageError(true); setMessage("⚠️ Please upload a profile picture."); return; }
    ...
    try {
      ...
      if (!token) { setMessage("⚠️ Failed to get verification token. Try again."); setLoading(false); return; }
        ...
        <Grid item xs={12} md={6}>
          <Paper elevation={4} sx={{ p:4, borderRadius:3 }}>
            ...
            <Box component="form" onSubmit={handleSubmit}>
              ...
              {/* Qualification */}
              <Autocomplete options={qualificationsList} value={formData.qualification || null}
              ...
              {higherEducation.includes(formData.qualification) && (
                <Autocomplete options={subjectsList} value={selectedHigherSubject||null}
                  onChange={(e,newValue)=>{ setSelectedHigherSubject(newValue||""); setFormData(p=>({...p, subject:newValue||""})); }}
                  renderInput={(params)=> <TextField {...params} label="Higher Qualification Subject" margin="normal" fullWidth /> }
                />
              )}

              {/* Major Subjects */}
              <Autocomplete multiple options={subjectsList} value={majorSubjects}
                onChange={(e,newValue)=>{ if(newValue.length>5) return; setMajorSubjects(newValue); setFormData({...formData, major_subjects:newValue.join(", ")}); }}
              ...
              {/* Experience */}
              <TextField label="Experience (Years)" name="experience" type="number" inputProps={{min:0,max:40}}
                value={formData.experience} onChange={(e)=>{let val=Number(e.target.value); if(val<0) val=0; if(val>40) val=40; setFormData({...formData, experience:val});}}
              ...
              {locationBlocked && <Box textAlign="center" mb={2}><Button variant="outlined" color="secondary" onClick={handleFindMe}>📍 Find My Location</Button></Box>}

              {/* Terms */}
              <FormControlLabel control={<Checkbox checked={formData.agree} onChange={handleChange} name="agree" color="success" />}
